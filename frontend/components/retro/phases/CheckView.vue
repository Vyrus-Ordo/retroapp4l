<template>
  <div class="panel p-8 flex flex-col gap-6 min-h-[50vh]">
    <h1 class="text-xl font-bold text-gray-900 mb-2">Check de ações anteriores</h1>
    <p class="text-gray-600">Revise as ações pendentes da última retro encerrada deste time.</p>
    <div class="mt-4 space-y-3">
      <div v-for="action in retroStore.previousActions.action_items" :key="action.id" class="rounded-lg border border-gray-100 bg-gray-50 p-4 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-2">
        <div>
          ...
          <p class="mt-1 text-xs text-gray-500">{{ action.assignee_name || 'Sem responsável' }}</p>
        </div>
